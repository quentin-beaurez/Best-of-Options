...
\begin{align*}
  ...
  \text{i.e.}\quad x &\leq \frac{\log\left(\frac{K}{s_0^i}\right) - \left(r - \frac{\sigma_i^2}{2}\right)T}{\sigma_i\sqrt{T}} = A_i\\
\end{align*}

Donc :

\begin{align*}
  P^{E,i} &= \int_{-\infty}^{A_i} e^{-rT}\Big(K - s^i_0e^{\left(r - \frac{\sigma_i^2}{2}\right)\,T + \sigma_i \sqrt{T} x}\Big)p(x) \d x,\\
          &= \int_{-\infty}^{A_i} K e^{-rT} \, p(x) \d x -  \int_{-\infty}^{A_i}s^i_0e^{- \frac{\sigma_i^2}{2}\,T + \sigma_i \sqrt{T} x}p(x) \d x,
  \shortintertext{En notant que $K e^{-rT}$ est une constante par rapport à $x$, on a :}
  P^{E,i} &=  K e^{-rT}\, F(A_i) - s^i_0\int_{-\infty}^{A_i}\frac{1}{\sqrt{2 \pi}} e^{- \frac{\sigma_i^2}{2}\,T + \sigma_i \sqrt{T} x - \frac{x^2}{2}} \d x.
...
\intertext{Notons également que $- \frac{\sigma_i^2}{2}\,T + \sigma_i \sqrt{T} x - \frac{x^2}{2} = -\frac{(x - \sigma_i\sqrt{T})^2}{2}$. Ainsi on a :}
   P^{E,i} &=   K e^{-rT}\, F(A_i) - s^i_0\int_{-\infty}^{A_i}\frac{1}{\sqrt{2 \pi}}e^{-\frac{(x - \sigma_i\sqrt{T})^2}{2}} \d x,\\
\shortintertext{puis, en faisant le changement de variable $y = x - \sigma_i\sqrt{T}$ on voit que le deuxième terme est aussi une fonction de répartition d'une normale $\N(0,1)$. }
  P^{E,i}  &=   K e^{-rT} \, F(A_i) - s^i_0\int_{-\infty}^{A_i - \sigma_i\sqrt{T}}\frac{1}{\sqrt{2 \pi}}e^{-\frac{y^2}{2}} \d y,\\
          &=  K e^{-rT}\, F(A_i) - s_0^i\,F(A_i - \sigma_i\sqrt{T} ).
\end{align*}
...
\implies  & \left( K - \max_i S^i_T \right)_+ \leq \left( K - \underset{i}{\text{moyenne}} S^i_T \right)_+ \\
\implies  & \left( K - \max_i S^i_T \right)_+ \leq \left( \underset{i}{\text{moyenne}} (K - S^i_T) \right)_+ \leq \underset{i}{\text{moyenne}} \left\lbrace \left( K - S^i_T \right)_+ \right\rbrace
\end{align*}

D'où, 

$$ P^2 = e^{-rT} \E\left(\left((K - \max_i(S^i_T)\right)_+\right) \leq   e^{-rT} \E \left( \dfrac{1}{n} \sum_{i=1}^n  \left\lbrace \left( K - S^i_T \right)_+ \right\rbrace \right) \leq  e^{-rT} \, \underset{i}{\text{moyenne}} P^{E, i} $$
...
P^1 &=  \E \left(\max\left(s_0^1 \exp\left(- \frac{\sigma_1^2}{2}\,T + \sigma_1W_T^1\right), s_0^2 \exp\left(- \frac{\sigma_2^2}{2}\,T + \sigma_2\sqrt{1-\rho^2}(B_t + \rho W_T^1)\right) \right)\right)\\
      &\qquad- e^{-rT}K,\\
      &=  \E \Bigg(\exp\left(\sigma_2 \rho \sqrt{1-\rho^2} W_T^1\right)\,\max\Bigg(s_0^1 \exp\left(- \frac{\sigma_1^2}{2}\,T + \left(\sigma_1 - \sigma_2 \rho \sqrt{1-\rho^2}\right) W_T^1\right), \\
  ...
  P^1 &=  \E \Bigg(\exp\left(\sigma_2 \rho \sqrt{1-\rho^2} \sqrt{T} X\right)\,\max\Bigg(s_0^1 \exp\left(- \frac{\sigma_1^2}{2}\,T + \left(\sigma_1 - \sigma_2 \rho \sqrt{1-\rho^2}\right) \sqrt{T} X\right), \\
      &\qquad s_0^2 \exp\left(- \frac{\sigma_2^2}{2}\,T + \sigma_2\sqrt{1-\rho^2}\sqrt{T} Y \right) \Bigg)\Bigg) - e^{-rT}K,\\
      &= \int_\R\int_\R \exp\left(\sigma_2 \rho \sqrt{1-\rho^2} \sqrt{T} x\right)\,\max\Bigg(s_0^1 \exp\left(- \frac{\sigma_1^2}{2}\,T + \left(\sigma_1 - \sigma_2 \rho \sqrt{1-\rho^2}\right) \sqrt{T} x\right),\\
  ...
  \shortintertext{où $p_X$ et $p_Y$ sont les densités (respectives) des v.a. $X$ et $Y$.}
      ...
      &\qquad + \int_\R \int_{A}^{+\infty}s^2_0 \exp\left(- \frac{\sigma_2^2}{2}T + \sigma_2 \sqrt{1-\rho^2}\sqrt{T}(y + \rho x)\right) p_X(x) \cdot p_Y(y)\d y\, \d x - e^{-rT}K,
        \shortintertext{où $A = \dfrac{(\sigma_1^2 - \sigma_2^2) + 2\sigma_1}{\rho\sqrt{1-\rho^2}\sqrt{T}} - \sigma_2x$.}
%   P^1 &= \E\left(S_T^2 \max\left(\frac{s_0^1}{s_0^2}\, \exp\left(-\frac{\sigma_1^2 - \sigma_2^2}{2}T + \left(\sigma_1W_T^1 - \sigma_2\sqrt{1-\rho^2}(B_T + \rho W_T^1)\right)\right),1\right)\right) - e^{-rT}K,\\
%      &= \E\left(S_T^2 \max\left(\frac{s_0^1}{s_0^2}\, \exp\left(-\frac{\sigma_1^2 - \sigma_2^2}{2}T + (\sigma_1 - \sigma_2\, \rho\sqrt{1-\rho^2}\,) \, W_T^1 - \sigma_2\sqrt{1-\rho^2}B_T\right)\right),1\right) - e^{-rT}K
 %       \intertext{Soit $ja$}
 %       &= \int_\R \int_R
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%      &=  E^{-rT} \E \left(\max\left(s_0^1 \exp\left((r- \frac{\sigma_1^2}{2})\,T + \sigma_1\sqrt{T} Y\right), s_0^2 \exp\left((r- \frac{\sigma_2^2}{2})\,T + \sigma_2\sqrt{T} Y\right) \right)\right)\\
%  &\qquad- e^{-rT}K,
%        \shortintertext{où $Y \sim \N(0,1)$.}
%      &= \int_\R \max\left(s_0^1\exp\left(-\frac{\sigma_1^2}{2}\,T + \sigma_1\sqrt{T} y\right), s_0^2 \exp\left(-\frac{\sigma_2^2}{2}\,T + \sigma_2\sqrt{T} y\right)\right)\, \frac{1}{\sqrt{2\,\pi}}\exp\left(-\frac{y^2}{2}\right) \d y \\
%      &\qquad - e^{-rT}K\\
%      &= \int_\R \max\left(\frac{s^1_0}{s^2_0}\exp\left(-\frac{\sigma_1^2 - \sigma_2^2}{2}T + (\sigma_1 - \sigma_2)\sqrt{T}y\right), 1\right) \cdot \left(s_0^2 \exp\left(-\frac{\sigma_2^2}{2}\,T + \sigma_2\sqrt{T} y\right)\right)\\
%      &\qquad \frac{1}{\sqrt{2\,\pi}}\exp\left(-\frac{y^2}{2}\right) \d y - e^{-rT}K\\
%  \intertext{Soit $g(y) = -\frac{\sigma_1^2 - \sigma_2^2}{2}T + (\sigma_1 - \sigma_2)\sqrt{T}y$}
%      &= \int_\R s_0^1\exp\left(-\frac{\sigma_1^2}{2}\,T + \sigma_1\sqrt{T} y\right) \1_{g(y) > \log\left(\frac{s_0^2}{s_0^1} \right)} \cdot \frac{1}{\sqrt{2\,\pi}}\exp\left(-\frac{y^2}{2}\right) \d y \\
%      &\qquad+ \int_\R s_0^1\exp\left(-\frac{\sigma_2^2}{2}\,T + \sigma_2\sqrt{T} y\right) \1_{g(y) \leq \log\left(\frac{s_0^2}{s_0^1} \right)} \cdot \frac{1}{\sqrt{2\,\pi}}\exp\left(-\frac{y^2}{2}\right) \d y  - e^{-rT}K\\
%      &= s_0^1 \int_\R \frac{1}{\sqrt{2\,\pi}} \exp\left(-\frac{(y - \sigma_1\sqrt{T})}{2}\right) \1_{g(y) > \log\left(\frac{s_0^2}{s_0^1} \right)} \d y \\
%      &\qquad+ s_0^2 \int_\R \frac{1}{\sqrt{2\,\pi}} \exp\left(-\frac{(y - \sigma_2\sqrt{T})}{2}\right) \1_{g(y) \leq \log\left(\frac{s_0^2}{s_0^1} \right)} \d y  - e^{-rT}K\\
\end{align*}
